...
mcp_server = FastMCP(
    ...
    instructions=(
        "Optimization-first Meta Ads MCP server. If you are unsure which tool "
        "to use, call get_capabilities with an intent key instead of exploring "
        "multiple tools. Start with health_check if auth or connectivity is "
        "uncertain, then use list_ad_accounts and discovery tools to find ids. "
        "Use get_account_pages before creative creation when a Page-linked "
        "asset is needed and list_instagram_accounts when an Instagram identity "
        "is needed. For detailed reporting use get_entity_insights. For "
